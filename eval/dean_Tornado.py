...
def get_rank(X, query_idx, target_idx, include_idx_mask=[]):
    indices = get_neighbors(X, query_idx, args.metric, include_idx_mask)
...
def load_embedding():
    ...
    embeddings = np.load(args.input_dir + "/embedding.npy")
    address_for_embedding = np.load(args.input_dir + "/address.npy")
